...
class NDimSurrogateEquilibSolver(NDimEquilibSolver):
    ...
    def __init__(
        ...
        self.X_train: List[List[float]] = []  # list of weight vectors (n_objs,)
        ...
        # Override the oracle_label used by the solver generator
    ...
    def solve_with_surrogate(self, max_iterations: int = 15) -> Optional[np.ndarray]:
        self._path_history = []
        
        # Batch-compatible oracle wrapper
        def surrogate_batch_oracle(weights_batch: torch.Tensor) -> torch.Tensor:
            batch_size = weights_batch.shape[0]
            labels = torch.zeros(batch_size, dtype=torch.long, device=weights_batch.device)
        ...
        for it in range(max_iterations):
            logger.info(f"Surrogate iteration {it+1}/{max_iterations} (real queries so far: {self.real_queries})...")
            
            try:
                result_tensor = self.solve(oracle_fn=surrogate_batch_oracle, batch_size=1)
                result = result_tensor[0].cpu().numpy()
            except Exception as e:
                logger.warning(f"Surrogate walk failed: {e}, sampling more random points...")
                for _ in range(3):
                    w = np.random.dirichlet(np.ones(self.n_objs))
                    self._query_real(w)
                self._train_surrogate()
            ...
            if lbl != sur_lbl:
                logger.info(f"Surrogate disagreed at candidate. Real: {lbl}, Surrogate: {sur_lbl}. Retraining...")
                self._train_surrogate()
            else:
                logger.info(f"Verified fixed point candidate. Real queries total: {self.real_queries}")
                return result
...
class SurrogateEquilibSolver(EquilibSolver):
    ...
    def __init__(self, subdivision: int = 20, n_init_samples: int = 10, real_cost_delay: float = 0.1):
        ...
        logger.info(f"Bootstrapping Legacy Surrogate with {n_init_samples} samples...")
        
        boundary_points = [
            (0, 0), (self.n, 0), (0, self.n), 
            (self.n//2, 0), (0, self.n//2), (self.n//2, self.n//2)
        ]
    ...
    def solve_with_surrogate(self, max_iterations: int = 20) -> Optional[List[Tuple[int, int]]]:
        logger.info(f"Starting Active Learning Loop (Max {max_iterations} refits)...")
        ...
        for i in range(max_iterations):
            ...
            if result_tri is None:
                logger.info("Surrogate could not find a path. Sampling random points.")
                for _ in range(3):
                    r = np.random.rand(3)
                    r /= r.sum()
                    x = int(r[0] * self.n)
            ...
            for pt in result_tri:
                ...
                if surrogate_label != real_label:
                    disagreement = True
                    logger.debug(f"Mismatch at {pt}: Surrogate={surrogate_label}, Real={real_label}")
            
            self._train_surrogate()
            
            if set(real_labels) == {0, 1, 2}:
                logger.info(f"Verified Fixed Point Found! Total Real Queries: {self.real_queries}")
            ...
            if disagreement:
                logger.info("Surrogate was wrong. Retraining and restarting search...")
