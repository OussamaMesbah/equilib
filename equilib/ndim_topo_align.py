        ...
        def safe_oracle(w_batch):
            ...
            for i in range(w_batch.shape[0]):
                ...
                if w[l] <= 0:
                    ...
                    if len(nonzero) > 0: labels[i] = nonzero[0]
            return labels

        # Initialize labels for initial simplex
        for k in range(self.n_objs):
            v = self.get_vertex_batch(y_base, sigma, k)
            simplex_labels[:, k] = safe_oracle(self.get_barycentric_weights(v))

        active_items = torch.ones(batch_size, device=self.device, dtype=torch.bool)
            ...
            for step in range(self.n_sub * (active_dim + 1) * 2):
                ...
                for i in range(batch_size):
                    ...
                    for k in range(active_dim + 1):
                        ...
                        face_labels = [cur_labels[j] for j in range(active_dim + 1) if j != k]
                ...
                sigma = torch.where(actual_active.unsqueeze(1), sigma_new, sigma)
                
                upd_idx = torch.where(actual_active & (pivot_k != -1))[0]
                for idx in upd_idx:
                    pk = pivot_k[idx].item()
                    new_v = self.get_vertex_batch(y_base[idx:idx+1], sigma[idx:idx+1], pk)
                    simplex_labels[idx, pk] = safe_oracle(self.get_barycentric_weights(new_v))
        ...
        for active_dim in range(1, self.d + 1):
            ...
            for step in range(self.n_sub * (active_dim + 1) * 2):
                ...
                for k in range(active_dim + 1):
                    ...
                    face = [cur_labels[j] for j in range(active_dim + 1) if j != k]
                    if set(face) == door_labels:
                        pivot_k = k
                        break
                if pivot_k == -1: return self.get_barycentric_weights(y_base)
                
                pk_t = torch.tensor([pivot_k], device=self.device, dtype=torch.long)
