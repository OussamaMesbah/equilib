...
class RLHFSteeringSolver(EquilibSolver):
    ...
    def __init__(self, subdivision=20, targets=None):
        ...
        self.targets = np.array(targets) if targets else np.array(
            [0.7, 0.8, 0.4])
        self.simulator = RLHFSteeringOracle()

        logger.info(
            f"RLHF Steering Objectives: Helpfulness={self.targets[0]}, Safety={self.targets[1]}, Verbosity={self.targets[2]}"
        )
    ...
    def oracle_label(self, x, y):
        ...
        # Standard Sperner uses (w - target)^2 logic for weights themselves.
