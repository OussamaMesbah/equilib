...
def main():
    ...
    def topo_oracle_label(w):
        # Sperner requires directional feedback:
    ...
    # ---------------------------------------------------------
    # Benchmark 2: Sperner (Sperner Walk)
    # ---------------------------------------------------------
    print("\n[2] Running Sperner (Gradient-Free Search)...")
    Metrics.evals = 0
    start_time = time.time()
    ...
    print("================================================================")
    print(
        f"{'Method':<20} | {'Loss':<10} | {'Evals':<8} | {'Improvement':<15}")
    print("-" * 65)
    print(
        f"{'Linear Merge':<20} | {linear_loss:.5f}    | {linear_evals:<8} | {'-'}"
    )
    print(
        f"{'Grid Search':<20} | {best_grid_loss:.5f}    | {grid_evals:<8} | {-(best_grid_loss-linear_loss)/linear_loss*100:.1f}%"
    )
    print(
